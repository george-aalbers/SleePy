        ...
        for target in targets[6:]:
            ...
            for pp in data.id.unique().tolist():
                ...
                if X_train.shape[0] > 9:
                    ...
                    model = RandomizedSearchCV(svr, random_grid, cv=5, n_iter=100, n_jobs=64, random_state = 0)

                    # Fit the model
                    model.fit(X_train, y_train)

                    # Save model
                    with open('/home/haalbers/sleep-estimation/gen/analysis/output/idiographic/models/svr_' + target + "_" + pp +  "_" + str(feature_set_number) + '.pkl','wb') as f:
                        pickle.dump(model,f)
                    ...
                    performance_single_target.to_csv("/home/haalbers/sleep-estimation/gen/analysis/output/idiographic/accuracy/svr_" + target +  "_" + str(feature_set_number) + ".csv")
                    ...
                    predictions_multiple_subjects.to_csv("/home/haalbers/sleep-estimation/gen/analysis/output/idiographic/predictions/svr_" + target + "_" + str(feature_set_number) + ".csv")

#####################################################
# Hyperparameter grid for support vector regression #
#####################################################
...
random_grid = { 
    ...
    "gamma": ["scale","auto"],
    "C": np.arange(0.1,100,20).tolist(),
    "epsilon": np.arange(0.1,0.9,0.2).tolist()
    }
print(random_grid)
